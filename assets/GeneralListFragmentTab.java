...
import de.bundestag.android.sections.committees.CommitteesDetailsMembersByFractionListAdapter;
...
public class GeneralListFragmentTab extends ListFragment {
	...
	// GeneralListFragmentTab.class.getName(), this);
	// }
	@Override
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		if (!((getActivity() instanceof PlenumSpeakersActivity) || (getActivity() instanceof PlenumDebatesActivity))) {
	...
	private void handleCommitteesDetailsNewsDetailsFragmentActivity() {
		...
		Cursor articlesCursor = committeesDatabaseAdapter.fetchCommitteesNews(DataHolder.committeesId); // pass
																										// id
		...
		((TextView) getActivity().findViewById(R.id.mainTitle)).setText(DataHolder.committeeName);
	...
	private void createTvCalenderLayout() {
		...
		try {
			PlenumStreamObject plenumStreamData = plenumParser.parseStream();
			Date d = new Date();
	...
	public void updateList(int index, int noOfDisableTabs) {
		try {
			...
			ArrayList<HashMap<String, Object>> items = PlenumTvViewHelper.createTv(output.get(0));
			items.remove(0);
	...
	private void handleVisitorsOffersList() {
		...
		final Cursor offersCursor = visitorsDatabaseAdapter.fetchOffers();
	...
	private void handleCommitteesDetailMembersByTaskList(int committeesId) {
		...
		setListAdapter(new CommitteesDetailsMembersByFractionListAdapter(getActivity(), new ArrayList<HashMap<String, Object>>(), 0, new ArrayList<MembersDetailsObject>(),
	...
	private void handleCommitteesDetailMembersByFractionList(int committeesId, String committeesIdString, String fractionName) {
		...
		setListAdapter(new CommitteesDetailsMembersByFractionListAdapter(getActivity(), membersByFraction, membersByFractionPresidentCount, presidents, subPresidents,
				committeesObject, fractionName, committeeMembersCount));
	...
	private void handleMembersSubpage(int subPageId, int index) {
		...
		if (subPageId == MembersSubpageMembersActivity.SUB_PAGE_FRACTION) {
			...
			count.setText(membersCursor.getCount() + "Mitglieder");
		} else {
	...
	public void onListItemClick(ListView listView, View view, int position, long id) {
		try {
			...
			Activity activity = getActivity();
			if (activity instanceof NewsActivity && DataHolder._oldPosition != position) {
				DataHolder.mLockScreenRotation(activity);
				onAktullActivityClick(listView, view, position, id, activity);
			} else if (activity instanceof VisitorsOffersActivity && DataHolder._oldPosition != position) {
			...
			} else if (activity instanceof VisitorsContactActivity && DataHolder._oldPosition != position) {
				CursorAdapter vListTemp = (CursorAdapter) getListAdapter();
				VisitorsListAdapter vList = (VisitorsListAdapter) vListTemp;
				DataHolder.rowDBSelectedIndex = position - 1;
				int type = vList.getItemViewType(position);
				if (type != vList.GALLERY_LAYOUT) {
					startActivity(new Intent(this.getActivity(), VisitorsOffersActivity.class));
				}
			} else if (activity instanceof VisitorsLocationsActivity && DataHolder._oldPosition != position) {
				CursorAdapter vListTemp = (CursorAdapter) getListAdapter();
				VisitorsListAdapter vList = (VisitorsListAdapter) vListTemp;
				int type = vList.getItemViewType(position);
				if (type != vList.GALLERY_LAYOUT) {
			...
			} else if (activity instanceof VisitorsNewsActivity && DataHolder._oldPosition != position) {
				DataHolder.mLockScreenRotation(activity);
			...
			if (activity instanceof VisitorsLocationsActivity || activity instanceof VisitorsOffersActivity || activity instanceof VisitorsNewsActivity && position != 0) {
				DataHolder.rowDBSelectedIndex = position - 1;
			}
	...
	private void onCommmitteActivityClcik(ListView listView, View view, int position, long id, Activity activity) {
		...
		if (DataHolder.isOnline(activity)) {
			...
			// Toast.makeText(this.getActivity(), "Internet not available",
	...
	public void finalize() throws Throwable {
		...
		// if (activity instanceof NewsActivity) {
		// NewsListAdapter a = (NewsListAdapter) getListView().getAdapter();
		// a.finalize();
		// } else if (activity instanceof VisitorsOffersActivity) {
		// VisitorsListAdapter a = (VisitorsListAdapter)
		...
		// a.finalize();
		// } else if (activity instanceof MembersListActivity) {
		// MembersListAdapter a = (MembersListAdapter)
		// getListView().getAdapter();
		// a.finalize();
		//
		// } else if (activity instanceof MembersSubpageMembersActivity) {
		...
		// a.finalize();
		// } else if (activity instanceof CommitteesActivity) {
		// CommitteesListAdapter a = (CommitteesListAdapter)
		// getListView().getAdapter();
		// a.finalize();
		//
		// } else if (activity instanceof MembersSubpageMembersActivity) {
		...
		// a.finalize();
		// } else if (activity instanceof CommitteesDetailsNewsActivity) {
		// CommitteesDetailsNewsListAdapter a =
		// (CommitteesDetailsNewsListAdapter) getListView().getAdapter();
		// a.finalize();
		//
		// } else if (activity instanceof VisitorsContactActivity) {
		// VisitorsListAdapter a = (VisitorsListAdapter)
		...
		//
		// } else if (activity instanceof PlenumTvActivity) {
		// PlenumTvListAdapter a = (PlenumTvListAdapter)
		// getListView().getAdapter();
		// a.finalize();
		//
		// } else if (activity instanceof
		// CommitteesDetailsMembersByFractionActivity) {
		// CommitteesDetailsMembersByFractionListAdapter a =
		// (CommitteesDetailsMembersByFractionListAdapter)
		// getListView().getAdapter();
		// a.finalize();
		//
		// } else if (activity instanceof CommitteesDetailsTasksActivity) {
		...
		// visitorsOffersDetailsFragment = null;
		// committeesDetailsFragment = null;
		// visitorsLocationsDetailsFragment = null;
		// visitorsNewsDetailsFragment = null;
		// visitorsOffersListFragment = null;
		// membersDetailsFragment = null;
		// committeeDetailsFragmentNews = null;
		// isFirstTime = true;
		// speakerTimer = null;
		// debateTimer = null;
	}
