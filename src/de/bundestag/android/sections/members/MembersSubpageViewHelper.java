...
public class MembersSubpageViewHelper {
	 ...
	 */
	public static ArrayList<HashMap<String, Object>> createMembersFractions(FragmentActivity activity) {
	 ...
	 * Used by general list fragment.
	 */
	public static ArrayList<HashMap<String, Object>> createMembersCities(FragmentActivity activity) {
		MembersDatabaseAdapter membersDatabaseAdapter = new MembersDatabaseAdapter(activity);
		membersDatabaseAdapter.open();
	 ...
	 * Used by general list fragment.
	 */
	public static ArrayList<HashMap<String, Object>> createMembersElections(FragmentActivity activity) {
	 ...
	 * Used by general list fragment.
	 */
	public static ArrayList createMembersSubpage(FragmentActivity activity, int subPageId, int index) {
		MembersDatabaseAdapter membersDatabaseAdapter = new MembersDatabaseAdapter(activity);
		membersDatabaseAdapter.open();
		...
		if (subPageId == MembersSubpageMembersActivity.SUB_PAGE_FRACTION) {
			if (index == 0) {
				...
				DataHolder.memberCount = (membersDatabaseAdapter.fetchAllFractions("CDU/CSU")).getCount();
				membersCursor = membersDatabaseAdapter.fetchFractionMembers("CDU/CSU");
				selected = "CDU/CSU";
			} else if (index == 1) {
				DataHolder.memberCount = (membersDatabaseAdapter.fetchAllFractions("SPD")).getCount();
				membersCursor = membersDatabaseAdapter.fetchFractionMembers("SPD");
				selected = "SPD";
			} else if (index == 2) {
				DataHolder.memberCount = (membersDatabaseAdapter.fetchAllFractions("FDP")).getCount();
				membersCursor = membersDatabaseAdapter.fetchFractionMembers("FDP");
				selected = "FDP";
			} else if (index == 3) {
				DataHolder.memberCount = (membersDatabaseAdapter.fetchAllFractions("DIE LINKE.")).getCount();
				...
				// "DIE GRÜNEN");
				DataHolder.memberCount = (membersDatabaseAdapter.fetchAllFractions("BÜNDNIS 90/DIE GRÜNEN")).getCount();
			...
			String city = MembersSubpageViewHelper.cityNames[index];
			...
			String election = MembersSubpageViewHelper.electionNames[index].substring(0, 3);
			membersCursor = membersDatabaseAdapter.fetchElectionMembers(election);
			selected = MembersSubpageViewHelper.electionNames[index];
		}
